...
class VideoColoriser:
    def __init__(self,grayVideo,colorCoordinates,colorValues,parameters):
        """grayImage should be of shape duration x width x height x 3, colorCoordinates n x 3 , 
        colorValues n x 3"""
        self.width,self.height,self.duration,d = grayVideo.shape
        self.grayVideo = grayVideo[:,:,:,0]
        self.grayCoordinates = np.indices([self.duration,self.width,self.height]).reshape(3,self.duration*self.width*self.height).T
        self.colorCoordinates = colorCoordinates
        self.colorValues = colorValues
        self.delta = parameters["delta"]
        self.sigma1 = parameters["sigma1"]
    ...
    def kernelColorise(self):
        video = np.zeros((self.duration,self.width,self.height,3))
        colors = ["R","G","B"]
        for i in range(3):
            print(f"Solving for {colors[i]} pixels ... ")
            KD = self.getK(self.colorCoordinates,self.colorCoordinates)
            n = self.colorCoordinates.shape[0]
            self.a_s = lag.solve(KD+self.delta*np.eye(n),self.colorValues[:,i])
            print("Solved for a_s. Getting layer pixels ... ")
            K2 = self.getK(self.grayCoordinates,self.colorCoordinates)
            # debugging
            # print(f"{KD.shape=}")
            # print(f"{n=}")
            ...
            # print(f"{K2 = }")
            layer_i = self.getK(self.grayCoordinates,self.colorCoordinates) @ self.a_s
            layer_i = layer_i.reshape(self.duration,self.width,self.height)
            video[:,:,:,i] = layer_i
        return video.astype(np.uint16)
    ...
    def getK(self,X,Y):
        """Generates the kernel matrix for 2 lists of indicies X and Y. X and Y 
        are lists of coordiantes of shape k x 3"""
        nx,ny = len(X),len(Y)
        # return np.random.rand(nx,ny)/1000
        distXY = lag.norm(X[:,np.newaxis]-Y[np.newaxis,:],axis=2)
        grayX = self.grayVideo[X[:,0],X[:,1],X[:,2]]
        grayY = self.grayVideo[Y[:,0],Y[:,1],Y[:,2]]
        grayXY = np.abs(grayX[:,np.newaxis]-grayY[np.newaxis,:])
        return self.kernel(distXY/self.sigma1)*self.kernel(grayXY**self.p/self.sigma1)
