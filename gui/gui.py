...
class DrawStuff(object):
    def __init__(self, mainwin,filepath):
        self.mainwin = mainwin
        self.label = tk.CTkLabel(self.mainwin, text=filepath)
        self.label.pack()
        # self.canvas = mainwin.canvas
        # self.axes = mainwin.axes
        # self.img = mainwin.rawImage

    def draw_line(self):
        print("hi")
        #print("hi")
         # self.axes[3] = self.axes[0]
         # self.canvas.draw()
...
class Application(tk.CTkFrame):
    ...
    def popupWindow(self):
        pp = tk.CTkToplevel(self)
        fig2 = plt.figure(figsize=(6, 4)) #TODO: what size?
        pp.ax = fig2.add_subplot(111)
        pp.ax.axis('off')
        ppcanvas = FigureCanvasTkAgg(fig2,master=pp)

        ppcanvas.get_tk_widget().grid(row=0, column=1)
        ppcanvas.draw()

        gs = GridSpec(2, 3)
        pp.ax01 = fig2.add_subplot(gs[0:2, 0:2])
        pp.ax11 = fig2.add_subplot(gs[0, 2])
        pp.ax21 = fig2.add_subplot(gs[1, 2])
        img = self.dimensionalise(self.rawImage,self.grayImage)
        # img = np.array(plt.imread(self.grayImage))
        wheel = plt.imread("images/color_wheel.jpeg")
        selected_color = np.ones([2, 2, 3]) * 0
        pp.ax01.imshow(img)
        pp.ax11.imshow(wheel)
        pp.ax01.axis('off')
        pp.ax11.axis('off')
        pp.ax21.axis('off')
        pp.ax21.imshow(selected_color)
        nx, ny, d = img.shape
        sc = [0, 0, 0]
        def onclick(event):
            global sc
            print("clicked")
            if event.inaxes==pp.ax01:
        ...
        # pp.ax.imshow(self.grayImage,cmap=)

        popupCloseButton = tk.CTkButton(pp, text="Close Window", command=pp.destroy)
        saveImageButton = tk.CTkButton(pp, text="Write to Main", command=self.saveImage)
        # self.saveButton = tk.CTkButton(master=root, text="Save Images", command=self.pop)
