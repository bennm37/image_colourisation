...
def compareColorisers(filename, nTests=10, nTrials=3, small=100, large=100):
    ...
    for i in range(nTests):
        ...
        c = Coloriser(
            grayImage, colorCoordinates, colorValues, parameters, verbose=False
        )
...
def plotResults(resultsName):
    ...
    ax[0].errorbar(
        df["nPixels"],
        df["averageFastTimes"],
        yerr=df["stdFastTimes"],
        label="Fast Times",
    )
    ax[0].scatter(df["nPixels"], df["averageLargeTimes"])
    ax[0].errorbar(
        df["nPixels"],
        df["averageLargeTimes"],
        yerr=df["stdLargeTimes"],
        label="Large Times",
    )
    ax[0].legend()
    ax[0].set_yscale("log")
    ...
    ax[1].errorbar(
        df["nPixels"],
        df["averageFastMemory"],
        yerr=df["stdFastMemory"],
        label="Fast Memory",
    )
    ax[1].scatter(df["nPixels"], df["averageLargeMemory"])
    ax[1].errorbar(
        df["nPixels"],
        df["averageLargeMemory"],
        yerr=df["stdLargeMemory"],
        label="Large Memory",
    )
    ax[1].legend()
    ax[1].set_yscale("log")
